...
class cache_flat_mutation_reader final : public flat_mutation_reader_v2::impl {
    ...
    bool can_gc(tombstone t) const {
        const auto max_purgeable = _max_purgeable_timestamp;
...
public:
    ...
    {
        _max_purgeable_timestamp = ctx.get_max_purgeable(dk);

        clogger.trace("csm {}: table={}.{}, dk={}, gc-before={}, max-purgeable-regular={}, reversed={}, snap={}",
                fmt::ptr(this),
                _schema->ks_name(),
                _schema->cf_name(),
                dk,
    ...
    if (_next_row_in_range) {
        ...
        if (_read_context.tombstone_gc_state() // do not compact rows when tombstone_gc_state is not set (used in some unit tests)
            ...
            auto row_tomb_expired = [&](row_tombstone tomb) {
                return (tomb && tomb.max_deletion_time() < _gc_before && can_gc(tomb.tomb()));
            };

            auto is_row_dead = [&](const deletable_row& row) {
                auto& m = row.marker();
                return (!m.is_missing() && m.is_dead(_read_time) && m.deletion_time() < _gc_before && can_gc(tombstone(m.timestamp(), m.deletion_time())));
            };
            ...
            if (row_tomb_expired(t) || is_row_dead(row)) {
                ...
                auto mutation_can_gc = can_gc_fn([this] (tombstone t) { return can_gc(t); });
