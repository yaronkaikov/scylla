...
from util import random_string, new_test_keyspace
...
async def test_streaming_deadlock_removenode(request, manager: ManagerClient):
    ...
    node1 = await manager.server_add(config=cfg, cmdline=cmdline)
    node2 = await manager.server_add(config=cfg, cmdline=cmdline)
    node3 = await manager.server_add(config=cfg, cmdline=cmdline)
    servers = [node1, node2, node3]
    cql = manager.get_cql()

    with new_test_keyspace(cql, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 2}") as ks:
