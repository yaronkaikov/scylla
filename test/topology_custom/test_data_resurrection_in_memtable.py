...
async def run_test_cache_tombstone_gc(manager: ManagerClient, statement_pairs: list[tuple[str]]):
    ...
    with new_test_keyspace(cql, "WITH replication = {'class': 'NetworkTopologyStrategy', 'replication_factor': 3}") as ks:
        ...
        def dump_mutation_fragments(description):
            for host in [host1, host2, host3]:
                ...
                logger.info("MUTATION_FRAGMENTS {} for {}:\n{}".format(description, host.address, '\n'.join(map(str, res))))
