...
from test.pylib.util import wait_for_view, wait_for, wait_for_cql_and_get_hosts

logger = logging.getLogger(__name__)

# A test for the view_flow_control_delay_limit_in_ms parameter.
#
# The test creates a table with a materialized view.
# we find a key such that the base replica is the first node, and the view replica is the second node.
#
...
async def test_configurable_mv_control_flow_delay(manager: ManagerClient) -> None:
    node_count = 2
    servers = [await manager.server_add(config={'error_injections_at_startup': ['update_backlog_immediately', 'skip_updating_local_backlog_via_view_update_backlog_broker']},
                                        cmdline=['--smp=1']) for _ in range(node_count)]
    cql = manager.get_cql()
    ...
    # find values for (key, c) such that a write to that partition triggers a remote
    # view update from srv_base
    async def find_key():
        while True:
            p, c = random.randint(0, int(1e6)), random.randint(0, int(1e6))
            view_update_pushed_remote_metric_name = 'scylla_database_total_view_updates_pushed_remote'
            metrics_before = await manager.metrics.query(srv_base.ip_addr)
            await cql.run_async(stmt, [p, c, ''], host=host_base)
            metrics_after = await manager.metrics.query(srv_base.ip_addr)
            if int(metrics_after.get(view_update_pushed_remote_metric_name) - metrics_before.get(view_update_pushed_remote_metric_name)) == 1:
                return p, c
    p, c = await find_key()
    ...
    for delay_limit in delay_limits:
        ...
        await cql.run_async(stmt, [p, c, 10000000*'a'], host=host_base)
        ...
        before_computed_delay = local_metrics.get(delay_metric_name, {'scheduling_group_name':'sl:default'}) or 0.0
        before_total_throttled_writes = local_metrics.get(throttled_writes_metric_name, {'scheduling_group_name':'sl:default'}) or 0.0
        ...
        local_metrics = await manager.metrics.query(srv_base.ip_addr)
        after_computed_delay = local_metrics.get(delay_metric_name, {'scheduling_group_name':'sl:default'}) or 0.0
        after_total_throttled_writes = local_metrics.get(throttled_writes_metric_name, {'scheduling_group_name':'sl:default'}) or 0.0
