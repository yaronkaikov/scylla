...
SEASTAR_TEST_CASE(test_multishard_reader_buffer_hint_large_partitions) {
    return do_with_cql_env_thread([] (cql_test_env& env) {
        ...
        const auto pkeys_by_tokens = boost::copy_range<std::map<dht::token, dht::decorated_key>>(ss.make_pkeys(2 * smp::count)
            | boost::adaptors::transformed([] (const dht::decorated_key& dk) { return std::pair(dk.token(), dk); }));
        ...
        {
            auto reader = make_flat_mutation_reader_from_mutations_v2(schema, semaphore.make_permit(), frozen_muts.front().front().unfreeze(schema), schema->full_slice());
            auto close_reader = deferred_close(reader);
        ...
        const auto reader_factory = [frozen_muts] (
            ...
            auto muts = boost::copy_range<std::vector<mutation>>(frozen_muts.at(this_shard_id())
                    | boost::adaptors::transformed([schema] (const frozen_mutation& fm) { return fm.unfreeze(schema); }));
            return make_flat_mutation_reader_from_mutations_v2(std::move(schema), std::move(permit), std::move(muts), ps);
        };
        ...
        auto run_test = [&] (size_t buffer_size, multishard_reader_buffer_hint buffer_hint,
            ...
            size_t shards_visited{0};
            {
                ...
                const auto shard_reader_buffer_size = buffer_hint ? buffer_size : flat_mutation_reader_v2::default_max_buffer_size_in_bytes();
        ...
        run_test(flat_mutation_reader_v2::default_max_buffer_size_in_bytes(), multishard_reader_buffer_hint::no);
        ...
        run_test(500 * 1024, multishard_reader_buffer_hint::no);
        run_test(500 * 1024, multishard_reader_buffer_hint::yes);
    });
}
...
SEASTAR_TEST_CASE(test_multishard_reader_buffer_hint_small_partitions) {
    return do_with_cql_env_thread([] (cql_test_env& env) {
        ...
        const auto pkeys_by_tokens = boost::copy_range<std::map<dht::token, dht::decorated_key>>(ss.make_pkeys(64 * smp::count)
            | boost::adaptors::transformed([] (const dht::decorated_key& dk) { return std::pair(dk.token(), dk); }));
        std::vector<std::vector<frozen_mutation>> frozen_muts(smp::count, std::vector<frozen_mutation>{});
        std::vector<lw_shared_ptr<reader_concurrency_semaphore>> semaphore_registry(smp::count, nullptr);
