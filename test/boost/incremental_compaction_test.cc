...
SEASTAR_TEST_CASE(incremental_compaction_test) {
    return sstables::test_env::do_with_async([&] (sstables::test_env& env) {
        ...
        auto do_incremental_replace = [&] (auto old_sstables, auto new_sstables, auto& expected_sst, auto& closed_sstables_tracker) {
            ...
            if (old_sstables.front()->generation().as_int() % 4 == 0) {
        ...
        for (auto i = 0U; i < tokens.size(); i++) {
            assert_that(sstable_reader(env.semaphore().make_tracking_only_permit(s.get(), "test reader", db::no_timeout, tracing::trace_state_ptr()), result[i], s))
