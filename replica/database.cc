...
database::database(const db::config& cfg, database_config dbcfg, service::migration_notifier& mn, gms::feature_service& feat, const locator::shared_token_metadata& stm,
    ...
    , _user_sstables_manager(std::make_unique<sstables::sstables_manager>("user", *_large_data_handler, *_corrupt_data_handler, _cfg, feat, _row_cache_tracker, dbcfg.available_memory, sst_dir_sem.local(), [&stm]{ return stm.get()->get_my_id(); }, dbcfg.streaming_scheduling_group, &sstm))
    , _system_sstables_manager(std::make_unique<sstables::sstables_manager>("system", *_nop_large_data_handler, *_nop_corrupt_data_handler, _cfg, feat, _row_cache_tracker, dbcfg.available_memory, sst_dir_sem.local(), [&stm]{ return stm.get()->get_my_id(); }, dbcfg.streaming_scheduling_group))
...
future<> database::create_local_system_table(
    ...
    if (!has_keyspace(ks_name)) {
        ...
        co_await create_keyspace(ksm, erm_factory, true, replica::database::system_keyspace::yes);
    }
...
future<> database::detach_column_family(table& cf) {
    ...
    _reader_concurrency_semaphores_group.foreach_semaphore([&] (scheduling_group sg, reader_concurrency_semaphore& sem) {
        return sem.evict_inactive_reads_for_table(uuid);
    });
}
...
api::timestamp_type memtable_list::min_live_timestamp(const dht::decorated_key& dk, api::timestamp_type max_seen_timestamp) const noexcept {
    const auto get_min_ts = [] (const memtable& mt) {
        // see get_max_purgeable_timestamp() in compaction.cc for comments on choosing min timestamp
        return mt.get_min_timestamp();
    };
