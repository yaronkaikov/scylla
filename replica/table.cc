...
static flat_mutation_reader_v2 maybe_compact_for_streaming(flat_mutation_reader_v2 underlying, const compaction_manager& cm,
...
future<>
table::compact_all_sstables(std::optional<tasks::task_info> info, do_flush do_flush, bool consider_only_existing_data) {
    if (do_flush) {
        co_await flush();
    }
