...
The default setting for the ``auto_snapshot_ttl`` flag in ``/etc/scylla/scylla.yaml`` file is ``0``. It is recommended to set it to a reasonable time allowing backup of the auto-snapshot, but to have a safety net that will automatically clean up stale snapshots to reclaim their disk space.
